...
def build_embed(track: Optional[wavelink.Playable], volume: int, paused: bool) -> discord.Embed:
    ...
    if track is None:
        e.title = "Now Playing"
        e.description = "No track currently playing"
        e.add_field(name="Volume", value=f"{volume}%", inline=True)
        return e
    e.title = "Now Playing"
    e.description = f"**[{track.title}]({track.uri})**"
    e.add_field(name="Author", value=track.author, inline=True)
    e.add_field(name="Duration", value=format_duration(track.length), inline=True)
    e.add_field(name="Status", value="Paused" if paused else "Playing", inline=True)
    if track.artwork:
...
class PlayerView(discord.ui.View):
    ...
    @discord.ui.button(label="Play/Pause", style=discord.ButtonStyle.success)
...
@bot.tree.command(name="play", description="Play music via Lavalink")
async def play(interaction: discord.Interaction, query: str):
    if not interaction.user.voice:
        return await interaction.response.send_message("You need to join a voice channel")
...
@bot.tree.command(name="stop", description="Leave the channel")
